...
// In-flight de-dupe: audio-options and stream requests for the same
// episode land within milliseconds of each other from the UI (one query
// per provider tab plus the actual stream fetch) — without this, both
// trigger their own full curl round-trip in parallel instead of sharing
// one, doubling real network time for no reason.
...
async function resolveSeries(anilistId, ctx = {}) {
  ...
  return dedupe(cacheKey, () => _resolveSeries(anilistId, cacheKey, ctx));
}

async function _resolveSeries(anilistId, cacheKey, ctx) {
  ...
  // Verify every candidate's AniList/MAL id concurrently instead of
  // sequentially — this was the main source of "opening a new anime is
  // slow": with N candidates it used to be N curl round-trips back to
  // back, now it's the single slowest one.
  const verified = await Promise.all(
    [...candidates.values()].map(async (candidate) => {
      const html = await fetchAnidbHtml(`${BASE}/anime/${candidate.slug}`, `${BASE}/home`).catch(() => "");
      if (!html) return null;
      return { candidate, ids: parseExternalIds(html), html };
    })
  );

  for (const v of verified) {
    if (!v || v.ids.anilistId !== Number(anilistId)) continue;
    const data = {
      slug: v.candidate.slug,
      siteId: v.candidate.siteId || Number(v.candidate.slug.match(/-(\d+)$/)?.[1]),
      title: parsePageTitle(v.html) || v.candidate.title,
      matchType: "anilist",
      matchScore: 1,
      ...v.ids,
    };
  ...
  if (malId) {
    for (const v of verified) {
      if (!v || v.ids.anilistId || v.ids.malId !== Number(malId)) continue;
      const data = {
        slug: v.candidate.slug,
        siteId: v.candidate.siteId || Number(v.candidate.slug.match(/-(\d+)$/)?.[1]),
        title: parsePageTitle(v.html) || v.candidate.title,
...
// Episode list + per-episode language list were being re-fetched from
// anidb.app on every single request (audio-options AND stream both called
// this independently) — cached now, since neither changes within a short
// window, so only the very first click on a given episode/series pays the
// network cost.
const EPISODES_TTL  = 15 * 60_000; // episode rows can gain filler/meta edits; keep this short
const LANGUAGES_TTL = 6 * 60 * 60_000; // language availability for an already-aired episode is effectively static

async function fetchProviderEpisodes(siteId) {
  const cacheKey = `np:anidbapp:eps:${siteId}`;
  const cached = get(cacheKey);
  if (isFresh(cached)) return cached.data;
  return dedupe(cacheKey, async () => {
    const data = await fetchJson(`${BASE}/api/frontend/anime/${siteId}/episodes`, `${BASE}/anime/${siteId}`);
    const episodes = Array.isArray(data.episodes) ? data.episodes : [];
    set(cacheKey, episodes, EPISODES_TTL);
    return episodes;
  });
}
...
// Resolve the specific provider episode row for a caller-facing episode
// number — shared by handleWatch and getAudioOptions so both always look at
// the exact same row (and therefore the exact same language list).
async function resolveProviderEpisode(anilistId, epNum, ctx = {}) {
  ...
  return { series, providerEp, episode };
}

async function handleWatch(anilistId, audio, epNum, ctx = {}) {
  const { series, providerEp, episode } = await resolveProviderEpisode(anilistId, epNum, ctx);
  ...
  // `audio` is either the legacy "sub"/"dub" or, when the caller picked a
  // specific track from getAudioOptions()'s real list (e.g. a second dub
  // language AniDB happens to carry), the exact language code — match that
  // exactly instead of forcing it through the sub/dub-only heuristic.
  const language =
    audio === "sub" || audio === "dub"
      ? languageForAudio(languages, audio)
      : languages.find((l) => String(l.code ?? "").toLowerCase() === audio.toLowerCase());
  ...
  return json({ anilistId: Number(anilistId), episode: Number(epNum), providerEpisode: providerEp, audio, language: language.code, languageName: language.name ?? language.code, streams });
}

// Real per-episode sub/dub availability from AniDB.app — not filtered down
// arbitrarily, but normalized to the "sub"/"dub" codes the rest of the
// stream pipeline (route matchers, provider fallback, findProviderForAudio)
// actually understands end-to-end. AniDB's own language list can carry
// more than two entries in principle, but every non-Japanese track is a
// dub of the same show, so it collapses to one real "dub" option — this
// still reflects genuine per-episode availability (an episode can list
// Japanese only, with no dub entry at all) rather than a hardcoded pair.
export async function getAudioOptions(anilistId, epNum, ctx = {}) {
  const { series, episode } = await resolveProviderEpisode(anilistId, epNum, ctx);
  if (!episode) return [];
  const languages = await fetchLanguages(episode.id, series.slug);
  const withEmbed = languages.filter((l) => l.embed_url);
  const options = [];
  if (withEmbed.some((l) => ["jpn", "ja", "japanese"].includes(String(l.code ?? "").toLowerCase()))) {
    options.push({ code: "sub", label: "Japanese" });
  }
  if (withEmbed.some((l) => !["jpn", "ja", "japanese"].includes(String(l.code ?? "").toLowerCase()))) {
    options.push({ code: "dub", label: "English" });
  }
  return options;
}
